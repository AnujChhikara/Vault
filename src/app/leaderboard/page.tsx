...
<div className='bg-black text-white min-h-screen py-12 flex flex-col items-center'>
      <h1 className='text-center text-3xl font-bold mb-8  text-transparent bg-clip-text bg-gradient-to-r from-pink-700 to-pink-200' >Leaderboard</h1>
        {
          isFethed ? <table className="table-auto sm:w-[330px]  md:w-[600px]">
        <thead className=''>
        ...
        <tbody className=' border-dotted border-2  border-zinc-400'> 
          ...
          {data.length > 0 && data.sort((a:any, b:any) => b.totalVotes - a.totalVotes).map((item:any, index:any) => (
            <tr key={item.user._id} className='text-center sm:text-sm md:text-lg font-semibold'>
              <td className="px-4 py-2">{index + 1 ==1 ? '🥇' : index + 1 ==2 ? '🥈' : index + 1 ==3 ? '🥉' : index + 1}</td>
              <td className="px-4 py-2">{item.user.name}</td>
              <td className=" px-4 py-2">{item.totalVotes}</td>
              <td className=" px-4 py-2">
                <Link href={`/profile/${item.user.name}`} className="text-pink-300 text-sm underline hover:underline">Profile</Link>
              </td>
            </tr>
          ))}
        </tbody>
